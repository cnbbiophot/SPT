function [trajectories] = plot_tracks_v20 (filesave, moleculeTrackDataArray, imSizeX, imSizeY, pixelSize)
%% Plot tracks
...
n_tracks = numel(moleculeTrackDataArray.coords);
colors = hsv(n_tracks);
...
for i_track = 1 : n_tracks
    clear molecule
    molecule(:,2:3)=moleculeTrackDataArray.coords{i_track,1}(:,1:2);
    molecule(:,1)=moleculeTrackDataArray.time{i_track,1}(:,1);
    frames=numel(moleculeTrackDataArray.frames{i_track,1});

    plot(molecule(:,2)/1000, molecule(:,3)/1000, 'Color', colors(i_track, :),'LineWidth',4)
