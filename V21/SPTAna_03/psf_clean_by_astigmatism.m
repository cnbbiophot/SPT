function [DataFilterAstg] = psf_clean_by_astigmatism(Input, DataPerFrame)
...
localisation=1;
for frame=1:numFrames
    if not(isempty(DataFilterAstg.psf{frame})) %si validLocalisation(frame)>0
        numLocalisations=size(DataFilterAstg.psf{frame,1},1);
