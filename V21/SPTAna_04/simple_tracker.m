function [ tracks adjacency_tracks A ] = simple_tracker(fileID, points, varargin)
...
if debug
    fprintf(fileID,'\n               Frame to frame linking using: %s method.', method);
end
...
fprintf(fileID,'\n               Creating %d links over a total of %d points for consecutives frames.', numel(link_flag), n_total_cells);


A = sparse(row_index, column_index, link_flag, n_total_cells, n_total_cells);
linkConsecutives=numel(link_flag);
...
for i = 1 : n_slices-2
    ...
    for j = i + 2 : min(i +  max_gap_closing, n_slices)
        ...
        for k = 1 : numel(target_indices)
            ...
            %                 if debug
            %                     fprintf(fileID,'Creating a link between point %d of frame %d and point %d of frame %d.\n', ...
            ...
            linkNonConsecutives=linkNonConsecutives+1;
...
fprintf(fileID,'\n               Creating %d links over a total of %d points for non-consecutives frames.', linkNonConsecutives, n_total_cells);
...
totalLinks=linkNonConsecutives+linkConsecutives;
fprintf(fileID,'\n               Created %d links over a total of %d points for all frames.', totalLinks, n_total_cells);
